...
TOOL_REGISTRY={
 "semantic_memory_search":{"risk":"LOW","description":"Search institutional memory semantically"},
 "evidence_verify":{"risk":"LOW","description":"Verify claims against internal evidence"},
 "business_read":{"risk":"LOW","description":"Read approved business intelligence data"},
 "experiment_execute":{"risk":"MEDIUM","description":"Execute approved strategic experiment"},
 "external_action":{"risk":"HIGH","description":"Perform external business action"}
}
def authorize_agent_tool(tool_name,agent=None,approved=False):
 tool=TOOL_REGISTRY.get(tool_name)
 if not tool:return {"allowed":False,"reason":"UNKNOWN_TOOL"}
 risk=tool["risk"]
 if risk=="LOW":return {"allowed":True,"reason":"LOW_RISK_READ"}
 if risk=="MEDIUM":return {"allowed":bool(approved),"reason":"APPROVAL_REQUIRED"}
 return {"allowed":False,"reason":"FOUNDER_CONTROLLED_ACTION"}
...
TOOL_REGISTRY={
 "institutional_memory_search":{"risk":"LOW","description":"Search Dreamarts institutional memory"},
 "evidence_verify":{"risk":"LOW","description":"Verify claims against available evidence"},
 "scenario_simulate":{"risk":"MEDIUM","description":"Run strategic scenario analysis"}
}
def agent_tool_policy(tool_name):
 t=TOOL_REGISTRY.get(tool_name)
 if not t:return {"allowed":False,"reason":"UNKNOWN_TOOL"}
 return {"allowed":True,"tool":tool_name,**t}
...
def ai_tool_catalog():
 return {
  "memory_search":{"risk":"LOW","description":"Search institutional semantic memory"},
  "evidence_verify":{"risk":"LOW","description":"Verify claims against stored evidence"},
  "scenario_simulate":{"risk":"LOW","description":"Run strategic scenario simulation"},
  "outcome_record":{"risk":"MEDIUM","description":"Record measured business outcome"}
 }

def execute_ai_tool(tool_name,args,token):
 catalog=ai_tool_catalog()
 if tool_name not in catalog:return {"ok":False,"error":"UNKNOWN_TOOL"}
 if catalog[tool_name]["risk"]!="LOW":return {"ok":False,"error":"APPROVAL_REQUIRED","risk":catalog[tool_name]["risk"]}
 if tool_name=="memory_search":
  q=vector_embedding(args.get("query",""));rows=ai_repo_select("ai_memory_vectors","*",token,limit=100) or []
  scored=sorted([{"text":r.get("text"),"metadata":r.get("metadata",{}),"similarity":round(cosine_similarity(q,r.get("embedding") or []),4)} for r in rows],key=lambda x:x["similarity"],reverse=True)
  return {"ok":True,"results":scored[:int(args.get("top_k",5))]}
 if tool_name=="evidence_verify":
  memory=ai_repo_select("ai_memory_vectors","*",token,limit=200) or []
  outcomes=ai_repo_select("ai_decision_outcomes","*",token,limit=100) or []
  claims=args.get("claims") or ([args.get("claim")] if args.get("claim") else [])
  return {"ok":True,"results":[verify_ai_claim(c,memory+outcomes) for c in claims]}
 return {"ok":False,"error":"NOT_IMPLEMENTED"}

@app.get("/api/admin/ai/tools")
def list_ai_tools():
 return jsonify(ok=True,tools=ai_tool_catalog())
...
TOOL_REGISTRY={
 "institutional_memory":{"risk":"LOW","description":"Search stored institutional memory"},
 "semantic_memory":{"risk":"LOW","description":"Search semantic vector memory"},
 "evidence_verify":{"risk":"LOW","description":"Verify claims against stored evidence"},
 "decision_outcomes":{"risk":"LOW","description":"Read historical decision outcomes"},
 "agent_performance":{"risk":"LOW","description":"Read agent calibration performance"}
}

def agent_tool_policy(tool_name,agent=None):
 spec=TOOL_REGISTRY.get(tool_name)
 if not spec:return {"allowed":False,"reason":"UNKNOWN_TOOL"}
 return {"allowed":spec["risk"]=="LOW","tool":tool_name,"risk":spec["risk"],"agent":agent,"reason":"READ_ONLY_TOOL_ALLOWED" if spec["risk"]=="LOW" else "APPROVAL_REQUIRED"}
